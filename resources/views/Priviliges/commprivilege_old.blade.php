				...
				@foreach($temp as $t)
					...
					@if($t->group_desc == $s->group_desc)
						<tr>
							...
							<td> 
								<select {{$parm=='S'?'': 'disabled'}} class="form-control" id="staff_deci_id{{$t->templdetno}}" name="staff_deci_id{{$t->templdetno}}">
		                                <option disabled selected value> -- select one  -- </option>
		                                @foreach($DecTyp as $dt)
		                                    <option  {{old(('staff_deci_id'.$t->templdetno),$t->staff_deci_id)==$dt->id? 'selected':''}}
